...
def find_files(top_dir, sub_dirs, extensions):
    ...
    for sub_dir in sub_dirs:
        dir = os.path.join(top_dir, sub_dir)
        if path.exists(dir):
            for root, _, file_names in os.walk(dir, onerror=lambda e: exit(f"Walk error: {e}")):
                for file_name in file_names:
...
def main(argv):
    ...
    source_file_names = find_files(
        cwd,
        ["libevmasm", "liblangutil", "solidity/libsolc", "libsolidity", "libsolutil", "libyul", "solppc"],
        [".h", ".cpp"]
    )
