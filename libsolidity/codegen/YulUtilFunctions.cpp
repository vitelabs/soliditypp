 ...
 * @date 2021
 * Solidity++: Component that can generate various useful Yul functions.
...
string YulUtilFunctions::leftAlignFunction(Type const& _type)
{
	...
	return m_functionCollector.createFunction(functionName, [&]() {
		...
		case Type::Category::ViteTokenId:
			templ("body", "aligned := " + leftAlignFunction(IntegerType(80)) + "(value)");
			break; 
...
string YulUtilFunctions::conversionFunction(Type const& _from, Type const& _to)
{
	...
	return m_functionCollector.createFunction(functionName, [&]() {
		...
		case Type::Category::ViteTokenId:
			body =
				Whiskers("converted := <convert>(value)")
					("convert", conversionFunction(IntegerType(80), _to))
					.render();
			break;
			...
			// Solidity++: 168-bit address
				...
				body =
					Whiskers("converted := <convert>(value)")
						...
						.render();
			// Solidity++: Vite token id
			else if (toCategory == Type::Category::ViteTokenId)
				body =
					Whiskers("converted := <convert>(value)")
						("convert", conversionFunction(_from, IntegerType(80)))
						.render();
			else
			{
				solAssert(
...
string YulUtilFunctions::cleanupFunction(Type const& _type)
{
	...
	return m_functionCollector.createFunction(functionName, [&]() {
		...
		// Solidity++: Vite token id
		case Type::Category::ViteTokenId:
			templ("body", "cleaned := " + cleanupFunction(IntegerType(80)) + "(value)");
			break;
