...
ExecutionBehavior ASTJsonImporter::executionBehavior(Json::Value const& _node)
{
    astAssert(member(_node, "executionBehavior").isString(), "ExecutionBehavior' expected to be string.");
    string const str = member(_node, "executionBehavior").asString();

    if (str == "sync")
        return ExecutionBehavior::Sync;
    else if (str == "async")
        return ExecutionBehavior::Async;
    else
        astAssert(false, "Unknown executionBehavior");
}
