	...
	char const* sourceCode = R"(
		contract C {
			function f() public view returns(bool) {
				(bool success,) = address("vite_010203040506070809080706050403020102030412227c8b71").staticcall("");
				return success;
			}
		}
	)";
		...
		char const* sourceCode = R"(
			contract C {
				function f() public view {
					address("vite_010203040506070809080706050403020102030412227c8b71").staticcall.value;
				}
			}
		)";
...
BOOST_AUTO_TEST_CASE(address_call_full_return_type)
{
	char const* sourceCode = R"(
		contract C {
			function f() public {
				(bool success, bytes memory m) = address("vite_010203040506070809080706050403020102030412227c8b71").call("");
				success; m;
			}
		}
	)";
...
BOOST_AUTO_TEST_CASE(address_delegatecall_full_return_type)
{
	char const* sourceCode = R"(
		contract C {
			function f() public {
				(bool success, bytes memory m) = address("vite_010203040506070809080706050403020102030412227c8b71").delegatecall("");
				success; m;
			}
		}
	)";
...
BOOST_AUTO_TEST_CASE(address_staticcall_full_return_type)
{
	if (solidity::test::CommonOptions::get().evmVersion().hasStaticCall())
	{
		char const* sourceCode = R"(
			contract C {
				function f() public view {
					(bool success, bytes memory m) = address("vite_010203040506070809080706050403020102030412227c8b71").staticcall("");
					success; m;
				}
			}
		)";
